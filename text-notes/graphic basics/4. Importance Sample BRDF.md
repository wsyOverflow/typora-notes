...
$$
想要求上述积分，除了很多采样点之外，还需要知道 $\boldsymbol{l}$ 的概率密度 $p(\boldsymbol{l})$，而这个概率密度与微表面法线分布**有关**。

### 2. Microfacet BRDF

这里只关注法线分布模型 NDF，微表面 BRDF 的详细细节请看 [[1]](#[1])。
...
#### 2.3 GGX(Trowbridge-Reitz) NDF

$$
D_{GGX}(\boldsymbol{h})=\frac{\alpha^2}{\pi((\cos\theta_h)^2(\alpha^2-1)+1)^2}
$$

## 根据 NDF 得到蒙特卡洛积分的概率密度

求 $\eqref{BRDF MC}$ 中的积分，需要一个关于入射方向的概率密度函数 $p(\boldsymbol{l})$。$p(\boldsymbol{l})$ 无法直接获取，需要通过微表面法线概率密度函数得到。 因此第一步是先求得 $p(\boldsymbol{h})$。
