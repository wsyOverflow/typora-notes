...
正交投影将 $(-l,r)\cross(-b,t)\cross(-n,-f)$ 的视锥体变换为 $(-1,1)\cross(-1,1)\cross(-1,1)$。注意：**通常情况下正交投影的视锥体定义为关于 $z$ 轴对称**，即 $l=r,b=t$，在本文最后结果对应替换即可。正交投影的过程可分为两步：先将视锥体近平面的中心平移至相机坐标系的原点；再将大小进行缩放。
...
经过平移后的视锥体为 $(\frac{-l-r}{2},\frac{l+r}{2})\cross(\frac{-b-t}{2},\frac{b+t}{2})\cross(\frac{n-f}{2},\frac{f-n}{2})$，将平移后的视锥体缩放为
 $(-1,1)\cross(-1,1)\cross(-1,1)$，构建缩放矩阵
$$
M_{orthoS}=
\begin{pmatrix}
\frac{2}{l+r} & 0 & 0 & 0 \\
...
Direct3D 与 Vulkan 的深度范围为 $[0,1]$，因此对应 clip space 的 view volume 为 $(-1,1)\cross(-1,1)\cross(0,1)$。因此需要修改正交投影变换：
...
2. 然后缩放平移后的视锥体 $(\frac{-l-r}{2},\frac{l+r}{2})\cross(\frac{-b-t}{2},\frac{b+t}{2})\cross(0,-(f-n))$ 为 $(-1,1)\cross(-1,1)\cross(0,1)$
   $$
   M_{orthoS}=
   \begin{pmatrix}
   \frac{2}{l+r} & 0 & 0 & 0 \\
...
​			即变换后的像素深度
$$
p.z= \frac{f\cdot z + nf}{n-f}\cdot \frac{1}{-z} \\
z=\frac{-nf}{(n-f)p.z+f}
$$
